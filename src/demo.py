from src.models.match_agg.match_aggregation import aggregate_match_xg, build_match_comparison, \
    summarize_win_prediction_accuracy, add_draw_band
...
MODEL_PATH = Path(__file__).resolve().parents[0] / "models" / "saved_models" / "best_xg_model.pkl"

model = XGBoostXGModel()
model.load(str(MODEL_PATH))

shot_df = ShotCleaningRunner().load_output()
shot_df = shot_df.rename(columns={'ball_speedmissing': 'ball_speed_missing'})
...
print(f"Best draw threshold: {threshold}")  # <- added
